...
//used to maintain tournament info
...
exports.parse = {
	...
	message: function(message, room) {
		...
		switch (spl[1]) {
			...
			case 'tournament':
				...
				switch (spl[2]) {
					...
					case 'battlestart':
						...
						var player1 = toId(spl[3]);
						var player2 = toId(spl[4]);
						var battleId = spl[5];
						scout.createBattle(player1, player2, battleId);
						break;
					case 'battleend':
					...
					case 'update':
						...
						//check for finals to start
						mmc.onUpdate(tourProgress, room);
						//scout.collectPlayersInTour(tourProgress)
						break;
					case 'updateEnd': 
						break;
					case 'forceend':
			 ...
			 * Battle Chat Cases*
			 * ******************/
			 // "room" is the battleId to be passed to the coreChallenge functions
			case 'init':
				// spl[2] is 'battle'
				break;
			case 'title':
			...
			case 'poke':
				...
				var pokeInfo = spl[3].split(', ');
				scout.addPokemon(room, spl[2], toId(pokeInfo[0]));
				break;
			case 'teampreview':
				// no spl[2]
